...
#
# Homework: Calculate Long Features
#
##############################################################################
...
# for each gesture with the 22 feature of the xml file stroke.
############################################################################
...
# Stoke class has 22 features and a list of points with (x, y, time) tuple
# infomation
...
class Stroke:
    def __init__(self):
        ...
        self.f12 = 0.0 # aspect [abs(45 - #4)]
        self.f13 = 0.0 # curviness, sum of angles that abs is less than 19
        self.f14 = 0.0 # total angle traversed / total length
        self.f15 = 0.0 # density metric 1 [#8/#5]
        self.f16 = 0.0 # density metric 2 [#8/#3]
        self.f17 = 0.0 # non-subjective "openness" [#5/#3]
        self.f18 = 0.0 # area of bounding box
        self.f19 = 0.0 # Log(area)
        self.f20 = 0.0 # total angle / total absolute angle
        self.f21 = 0.0 # Log(total length)
        self.f22 = 0.0 # Log(aspect)
    ...
    def calculate(self, x, y, t, points):
        ...
        # calculate f12
        self.f12 = 0.0
        self.f12 = math.fabs(math.atan(1) - self.f4)

        # calculate f13
        self.f13 = 0.0
        degree_19 = 1.0 * 19 / 180 * math.pi
        for i in range(theta):
            cur_value = math.fabs(theta[i])
            if cur_value < degree_19: self.f13 += cur_value

        # calculate f14
        self.f14 = 1.0 * self.f9 / self.f8

        # calculate f15
        self.f15 = 1.0 * self.f8 / self.f5

        # calculate f16
        self.f16 = 1.0 * self.f8 / self.f3

        # calculate f17
        self.f17 = 1.0 * self.f5 / self.f3

        # calculate f18
        self.f18 = 1.0 * bbdxdist * bbdydist

        # calculate f19
        self.f19 = 1.0 * math.log(self.f18)

        # calculate f20
        self.f20 = 1.0 * self.f9 / self.f10

        # calculate f21
        self.f21 = 1.0 * math.log(self.f8)

        # calculate f22
        self.f22 = 1.0 * math.log(self.f12)

# Gesture class has 22 features' average value, their std dev, list of xml,
...
class Gesture:
    def __init__(self):
        ...
        self.sdf12 = 0.0 # aspect [abs(45 - #4)]
        self.sdf13 = 0.0 # curviness, sum of angles that abs is less than 19
        self.sdf14 = 0.0 # total angle traversed / total length
        self.sdf15 = 0.0 # density metric 1 [#8/#5]
        self.sdf16 = 0.0 # density metric 2 [#8/#3]
        self.sdf17 = 0.0 # non-subjective "openness" [#5/#3]
        self.sdf18 = 0.0 # area of bounding box
        self.sdf19 = 0.0 # Log(area)
        self.sdf20 = 0.0 # total angle / total absolute angle
        self.sdf21 = 0.0 # Log(total length)
        self.sdf22 = 0.0 # Log(aspent)
        # number of strokes
    ...
    def Process(self, xmllist):
        ...
        for i in range(self.numofstrokes):
            ...
            self.f14 += self.strokes[i].f14
            self.f15 += self.strokes[i].f15
            self.f16 += self.strokes[i].f16
            self.f17 += self.strokes[i].f17
            self.f18 += self.strokes[i].f18
            self.f19 += self.strokes[i].f19
            self.f20 += self.strokes[i].f20
            self.f21 += self.strokes[i].f21
            self.f22 += self.strokes[i].f22
        ...
        self.f14 = 1.0*self.f14/self.numofstrokes
        self.f15 = 1.0*self.f15/self.numofstrokes
        self.f16 = 1.0*self.f16/self.numofstrokes
        self.f17 = 1.0*self.f17/self.numofstrokes
        self.f18 = 1.0*self.f18/self.numofstrokes
        self.f19 = 1.0*self.f19/self.numofstrokes
        self.f20 = 1.0*self.f20/self.numofstrokes
        self.f21 = 1.0*self.f21/self.numofstrokes
        self.f22 = 1.0*self.f22/self.numofstrokes
        ...
        for i in range(self.numofstrokes):
            ...
            self.sdf14 += math.pow(self.strokes[i].f14-self.f14,2)
            self.sdf15 += math.pow(self.strokes[i].f15-self.f15,2)
            self.sdf16 += math.pow(self.strokes[i].f16-self.f16,2)
            self.sdf17 += math.pow(self.strokes[i].f17-self.f17,2)
            self.sdf18 += math.pow(self.strokes[i].f18-self.f18,2)
            self.sdf19 += math.pow(self.strokes[i].f19-self.f19,2)
            self.sdf20 += math.pow(self.strokes[i].f20-self.f20,2)
            self.sdf21 += math.pow(self.strokes[i].f21-self.f21,2)
            self.sdf22 += math.pow(self.strokes[i].f22-self.f22,2)
        ...
        self.sdf14 = math.sqrt(1.0*self.sdf14/self.numofstrokes)
        self.sdf15 = math.sqrt(1.0*self.sdf15/self.numofstrokes)
        self.sdf16 = math.sqrt(1.0*self.sdf16/self.numofstrokes)
        self.sdf17 = math.sqrt(1.0*self.sdf17/self.numofstrokes)
        self.sdf18 = math.sqrt(1.0*self.sdf18/self.numofstrokes)
        self.sdf19 = math.sqrt(1.0*self.sdf19/self.numofstrokes)
        self.sdf20 = math.sqrt(1.0*self.sdf20/self.numofstrokes)
        self.sdf21 = math.sqrt(1.0*self.sdf21/self.numofstrokes)
        self.sdf22 = math.sqrt(1.0*self.sdf22/self.numofstrokes)


    # print the result to the screen
    
    def Print(self):
        a = [self.f1, self.f2, self.f3, self.f4, self.f5, self.f6, self.f7, self.f8, self.f9, self.f10, self.f11, self.f12, self.f13, self.f14, self.f15, self.f16, self.f17, self.f18, self.f19, self.f20, self.f21, self.f22]
        b = [self.sdf1, self.sdf2, self.sdf3, self.sdf4, self.sdf5, self.sdf6, self.sdf7, self.sdf8, self.sdf9, self.sdf10, self.sdf11, self.sdf12, self.sdf13, self.sdf14, self.sdf15, self.sdf16, self.sdf17, self.sdf18, self.sdf19, self.sdf20, self.sdf21, self.sdf22]
        for i in range(22):
    ...
    def Write(self,file):
        for i in range(len(self.strokes)):
            ...
            file.write(str(self.strokes[i].f14)+',')
            file.write(str(self.strokes[i].f15)+',')
            file.write(str(self.strokes[i].f16)+',')
            file.write(str(self.strokes[i].f17)+',')
            file.write(str(self.strokes[i].f18)+',')
            file.write(str(self.strokes[i].f19)+',')
            file.write(str(self.strokes[i].f20)+',')
            file.write(str(self.strokes[i].f21)+',')
            file.write(str(self.strokes[i].f22)+'\n')
