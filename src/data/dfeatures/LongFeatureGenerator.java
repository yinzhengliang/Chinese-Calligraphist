...
public class LongFeatureGenerator {
	private static String invMatrix = Constant.MODEL_DIR + "Long/" + "invLong.txt";
	private static String training_folder = Constant.MODEL_DIR + "Long/" + "LongData";
	private static String output_file = Constant.MODEL_DIR + "Long/" + "Long_Final_Weight.txt";
	private static Map<String, List<Stroke>> templates = new HashMap<String, List<Stroke>>();
	
	private static Long longRecognizer = new Long();
	...
	public static void generate() {
		...
		for (Map.Entry<String, List<Stroke>> entry : templates.entrySet()) {
			...
			for (Stroke template : entry.getValue()) {
				Map<String, Double> features = longRecognizer.calculateFeatures(template);
